...
void State::makeMove(const Location &loc, int direction)
{
  ...
  grid[nLoc.row][nLoc.col].newid = grid[loc.row][loc.col].id;
  grid[loc.row][loc.col].id = -1;
};

void State::makeMove(const Location &loc, char direction)
{
  makeMove(loc, getDirection(direction));
};
...
int State::getDirection(char direction) {
  switch (direction) {
    case 'N': return 0;
    case 'E': return 1;
    case 'S': return 2;
...
Location State::getLocation(const Location &loc, char direction) {
  return getLocation(loc, getDirection(direction));
};
  ...
  else
  {
    ...
    while(is >> inputType)
    {
      ...
      else if(inputType == "a") //live ant square
      {
        ...
        if (player == 0) {
          if (state.grid[row][col].newid != -1) {
            state.grid[row][col].id = state.grid[row][col].newid;
            state.grid[row][col].newid = -1;
            state.allMyAnts[state.grid[row][col].id] = Location(row, col);
          }
          else {
            state.grid[row][col].id = state.antid++;
            state.allMyAnts.push_back(Location(row, col));
          }
